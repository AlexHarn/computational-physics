...
for i, N in enumerate(Ns):
    ...
    skiprows=1)
    ax.plot(theta, T_f_a, 'r-', label="über die Ableitung")
    ax.plot(theta, T_f_k, 'gx', label="über das Kreuzprodukt")
    ax.legend(loc=1)
    ax.set_title(r"$N = {}$ für den ferromgn Fall".format(N))
    ax.set_xlim(0, 360)
    ax.set_ylim(0, 0.45)
